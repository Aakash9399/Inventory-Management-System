...
class categoryClass:
    def __init__(self,root):
        ...
        lbl_name=Label(self.root,text="Enter Category Name",font=("goudy old style",30),bg='white',relief=RIDGE).place(x=50,y=100)
        
        txt_name=Entry(self.root,textvariable=self.var_name,font=("goudy old style",18),bg='light yellow',relief=RIDGE).place(x=50,y=170,width=300)
        
        btn_add=Button(self.root,text='ADD',command=self.add,font=("goudy old style",15),bg='blue',fg='black',cursor='hand2').place(x=360,y=170,width=150,height=30)
        ...
        cat_frame.place(x=700,y=60,width=380,height=140)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                cur.execute("select * from categories where name=?",(self.var_name.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category Name already assigned,try different",parent=self.root)
                else:
                    cur.execute("insert into categories (name) values(?)",(self.var_name.get(),))
                    con.commit()
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get()=="":
                ...
                cur.execute("select * from categories where cid=?",(self.var_cat_id.get(),))
                row=cur.fetchone()
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from categories where cid=?",(self.var_cat_id.get(),))
                        con.commit()
